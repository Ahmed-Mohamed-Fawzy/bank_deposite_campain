...
with open("prediction.json") as source:
...
st.sidebar.image("data_analysis.jpeg")
st.sidebar.subheader("Choose Your Favorite Predictor")

# Filters
model = st.sidebar.selectbox(
    "predictors", ["Random Forest", "Gradient Boost","XG Boost"]
)
...
with col2:
    ...
    emp_var_rate = st.number_input('emp_var_rate', df['emp_var_rate'].min(), df['emp_var_rate'].max())
    cons_price_idx = st.number_input('cons_price_idx', df['cons_price_idx'].min(), df['cons_price_idx'].max())
    cons_conf_idx = st.number_input('cons_conf_idx', df['cons_conf_idx'].min(), df['cons_conf_idx'].max())
    euribor3m = st.number_input('euribor3m', df['euribor3m'].min(), df['euribor3m'].max())
    nr_employed = st.number_input('nr_employed', df['nr_employed'].min(), df['nr_employed'].max())
...
fig = go.Figure(data=[go.Table(header=dict(values=['<b>Model<b>', '<b>F1 Score<b>', '<b>Recall<b>', '<b>Precision<b>'],
